...
Prefer a ready-to-use recipe over adapting the generic setup by hand:

- [ZimaOS](/self-hosting/zimaos): import the custom-app Compose file.
- [CasaOS](/self-hosting/casaos): import the customized-app Compose file (separate file and steps from ZimaOS).
- [Coolify](/self-hosting/coolify): deploy from Git with the Docker Compose build pack.
- [Dokploy](/self-hosting/dokploy): use the staged catalogue template or the manual Compose fallback.
- [Portainer](/self-hosting/portainer): deploy the stack with environment variables in the UI.
- [Dockge](/self-hosting/dockge): manage the stack from the canonical Compose file.
- [NixOS](/self-hosting/nixos): run the maintained module with sops secrets.

Prefer containers? The [binary](/self-hosting/binary) and [docker-run](/self-hosting/docker-run) pages cover non-Compose installs. Every install ends at the same place: [reverse proxy and public access](/self-hosting/reverse-proxy).
