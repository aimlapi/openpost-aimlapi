...
- Stale import recovery no longer reloads blindly. URL-less import failures
  consult SvelteKit's own deployment check (`updated.check()` against the
  non-immutable `_app/version.json`): a confirmed newer deployment reloads
  once per running build, a current deployment keeps the explicit retry, and
  a failed check stays unclassified. The app and marketing site share one bounded controller (`@openpost/telemetry` chunk recovery): one decision per failure across preload, boundary, and rejection events, a persisted per-asset and overall attempt budget with no time-window reset, and automatic reloads only with evidence (a missing first-party asset means deployment skew, a served asset means one transient retry). Offline, same-build, unclassified, and unpersistable-budget failures keep the error boundary's explicit retry. Rejected imports stay rejected for SvelteKit.
